...
ans=0
for i in range(nd):
    prv,now,nxt=polygon[(i+nd-1)%nd],polygon[i],polygon[(i+1)%nd]
    dist1=(nxt-now).length()
    dist2=(now-prv).length()
    ans+=dist1 + L*math.acos((nxt-now)*(now-prv)/dist1/dist2)
print(round(ans))
'''
lengths=[0]*nd
for i in range(1,nd):
...
res=int(sum(lengths)+((L)*(math.pi)*(2)))
print(res)
'''